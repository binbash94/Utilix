...
type RowIn = { apn: string; street_address?: string; county: string; state: string };
...
export default function BulkUpload() {
  ...
  const onFile = async (file: File) => {
    ...
    try {
      ...
      const json = XLSX.utils.sheet_to_json<RowIn>(first, { raw: false });
      setRows(json);
  ...
  const run = async () => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        out.push({ apn: r.apn, electric_available: false, electric_provider: null,
    ...
    <div className="space-y-3">
      ...
      {rows.length > 0 && (
        <div className="text-sm text-ui-subtext">
          Loaded <b>{rows.length}</b> rows. Columns expected: <code>apn, street_address, county, state</code>
        </div>
      )}
